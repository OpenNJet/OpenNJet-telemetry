...
#include "api/opentelemetry_njt_api.h"
...
{
    wsAgent.releaseDependency();
}


OTEL_SDK_STATUS_CODE opentelemetry_core_init(OTEL_SDK_ENV_RECORD* env, unsigned numberOfRecords, struct cNode *rootCN, char *prefix, size_t prefix_len)
{
    ...
    res = wsAgent.init(env, numberOfRecords, prefix, prefix_len);
