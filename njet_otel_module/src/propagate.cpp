...
static bool FindHeader(njt_http_request_t* req, nostd::string_view key, nostd::string_view* value) {
  njt_list_part_t* part = &req->headers_in.headers.part;
  njt_table_elt_t* h = (njt_table_elt_t*)part->elts;

  for (njt_uint_t i = 0;; i++) {
    if (i >= part->nelts) {
      ...
      part = part->next;
      h = (njt_table_elt_t*)part->elts;
      i = 0;
    }

    if (
      key.size() != h[i].key.len ||
      njt_strncasecmp((u_char*)key.data(), h[i].key.data, key.length()) != 0) {
      continue;
    }

    *value = FromNjtString(h[i].value);
    return true;
  }
...
static bool HasHeader(njt_http_request_t* req, nostd::string_view header) {
  nostd::string_view value;
  return FindHeader(req, header, &value);
}

class TextMapCarrierNjt : public opentelemetry::context::propagation::TextMapCarrier
{
public:
  TextMapCarrierNjt(OtelCarrier* carrier) : carrier_(carrier) {}
...
TracePropagationType GetPropagationType(njt_http_request_t* req) {
  OtelNjtLocationConf* config = GetOtelLocationConf(req);
...
opentelemetry::context::Context ExtractContext(OtelCarrier* carrier) {
  ...
  TextMapCarrierNjt textMapCarrier(carrier);
