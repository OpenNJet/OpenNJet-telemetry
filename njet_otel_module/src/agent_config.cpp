...
static bool SetupOtlpExporter(toml_table_t* table, njt_log_t* log, OtelNjtAgentConfig* config) {
  ...
  if (!hostVal.ok) {
    njt_log_error(NJT_LOG_ERR, log, 0, "Missing required host field for OTLP exporter");
    return false;
  }
  ...
  if (!portVal.ok) {
    njt_log_error(NJT_LOG_ERR, log, 0, "Missing required port field for OTLP exporter");
    return false;
  }
...
static bool SetupExporter(toml_table_t* root, njt_log_t* log, OtelNjtAgentConfig* config) {
  toml_datum_t exporterVal = toml_string_in(root, "exporter");

  if (!exporterVal.ok) {
    njt_log_error(NJT_LOG_ERR, log, 0, "Missing required exporter field");
    return false;
  }
  ...
  if (!exporters) {
    njt_log_error(NJT_LOG_ERR, log, 0, "Unable to find exporters table");
    return false;
  }
  ...
  if (exporter == "otlp") {
    ...
    if (!otlp) {
      njt_log_error(NJT_LOG_ERR, log, 0, "Unable to find exporters.otlp");
      return false;
    }
    ...
    njt_log_error(NJT_LOG_ERR, log, 0, "Unsupported exporter %s", exporter.c_str());
    return false;
  }
...
static bool SetupService(toml_table_t* root, njt_log_t*, OtelNjtAgentConfig* config) {
...
static bool SetupProcessor(toml_table_t* root, njt_log_t* log, OtelNjtAgentConfig* config) {
  toml_datum_t processorVal = toml_string_in(root, "processor");

  if (!processorVal.ok) {
    njt_log_error(NJT_LOG_ERR, log, 0, "Unable to find required processor field");
    return false;
  }
...
static bool SetupSampler(toml_table_t* root, njt_log_t* log, OtelNjtAgentConfig* config) {
  ...
  if (samplerNameVal.ok) {
    ...
    if (samplerName == "AlwaysOn") {
      ...
      if (ratio.ok) {
        ...
        njt_log_error(NJT_LOG_ERR, log, 0, "TraceIdRatioBased requires a ratio to be specified");
        return false;
      }
    } else {
      njt_log_error(NJT_LOG_ERR, log, 0, "Unsupported sampler %s", samplerName.c_str());
      return false;
    }
  }
...
bool OtelAgentConfigLoad(const std::string& path, njt_log_t* log, OtelNjtAgentConfig* config) {
  ...
  if (!confFile) {
    njt_log_error(NJT_LOG_ERR, log, 0, "Unable to open agent config file at %s", path.c_str());
    return false;
  }
  ...
  if (!scopedConf.table) {
    njt_log_error(NJT_LOG_ERR, log, 0, "Configuration error: %s", errBuf);
    return false;
  }
